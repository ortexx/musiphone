...
import { getCache, excludeCacheFromSong } from '../../lib/cache.js';
...
export default class Player extends Akili.Component {
  ...
  compiled() { 
    ...
    if(!window.cordova) {
      ...
      navigator.mediaSession.setActionHandler('previoustrack', this.setPrevSong.bind(this));
      navigator.mediaSession.setActionHandler('nexttrack', this.setNextSong.bind(this));
    }   
  }
  ...
  async loadSrcBrowser(song) {    
    await new Promise((resolve, reject) => {
      ...
      navigator.mediaSession.metadata = new MediaMetadata({
        title: parts[1],
        artist: parts[0],
        artwork: network.connection? [{ src: song.coverLink }]: undefined 
      });
